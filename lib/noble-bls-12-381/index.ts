...
async function hash_to_field(
  ...
  if (htfOptions.expand) {
    pseudo_random_bytes = await expand_message_xmd(
      msg,
      DST,
      len_in_bytes,
      htfOptions.hash
...
export class PointG2 extends ProjectivePoint<Fp2> {
  ...
  toHex(isCompressed = false) {
    ...
    if (isCompressed) {
      ...
      return (
        toPaddedHex(x_1, PUBLIC_KEY_LENGTH) +
        toPaddedHex(x_0, PUBLIC_KEY_LENGTH)
      );
    } else {
      if (this.equals(PointG2.ZERO)) {
