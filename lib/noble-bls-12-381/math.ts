...
function genInvertBatch<T extends Field<T>>(
  cls: FieldStatic<T>,
  nums: T[]
): T[] {
...
export class Fp2 implements Field<Fp2> {
  ...
  frobeniusMap(power: number): Fp2 {
    return new Fp2(
      this.c0,
      this.c1.multiply(FP2_FROBENIUS_COEFFICIENTS[power % 2])
    );
  }
  multiplyByB() {
    let c0 = this.c0;
...
export class Fp6 implements Field<Fp6> {
  ...
  multiply(rhs: Fp6 | bigint) {
    ...
    return new Fp6(
      ...
      t0.add(
        c1.add(c2).multiply(r1.add(r2)).subtract(t1.add(t2)).mulByNonresidue()
      ),
  ...
  frobeniusMap(power: number) {
    return new Fp6(
      ...
      this.c1
        .frobeniusMap(power)
        .multiply(FP6_FROBENIUS_COEFFICIENTS_1[power % 6]),
      this.c2
        .frobeniusMap(power)
        .multiply(FP6_FROBENIUS_COEFFICIENTS_2[power % 6])
    );
  }
...
export class Fp12 implements Field<Fp12> {
  ...
  finalExponentiate() {
    ...
    return t2_t5_pow_q2
      .multiply(t4_t1_pow_q3)
      .multiply(t6_t1c_pow_q1)
      .multiply(t7_t3c_t1);
  }
...
type Constructor<T extends Field<T>> = {
  new (...args: any[]): T;
} & FieldStatic<T> & {
    MAX_BITS: number;
  };
...
function isogenyMap<T extends Field<T>>(
  COEFF: [T[], T[], T[], T[]],
  x: T,
  y: T
): [T, T] {
...
export const isogenyMapG2 = (x: Fp2, y: Fp2) =>
  isogenyMap(ISOGENY_COEFFICIENTS_G2, x, y);
// 11-isogeny map from E' to E
export const isogenyMapG1 = (x: Fp, y: Fp) =>
  isogenyMap(ISOGENY_COEFFICIENTS_G1, x, y);
...
const ISOGENY_COEFFICIENTS_G2: [Fp2_4, Fp2_4, Fp2_4, Fp2_4] = [
  xnum,
  xden,
  ynum,
  yden,
